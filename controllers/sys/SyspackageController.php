...
class SyspackageController extends BaseController {
...
public function all($page = 1, $perpage = 10, $kw = '', $ajax = 0){
        ...
        $columns = Column::where('table_id', 'syspackages')->orderBy('sort', 'asc')->get();
        $kws = [];
        if ($kw) {
            $kws = explode(',', $kw);
        }
        $qry = Syspackage::query();
        $qry->whereRaw('1 = 1');
        $vkw = '';
        if ($kws) {
        ...
        $info = Dbinfo::where('table_name', 'syspackages')->first();
        //---addition----
        $method = [];
        $domains = [];
