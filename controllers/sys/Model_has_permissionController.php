...
class Model_has_permissionController extends BaseController {
...
public function all($page = 1, $perpage = 10, $kw = '', $ajax = 0){
        ...
        $columns = Column::where('table_id', 'model_has_permissions')->orderBy('sort', 'asc')->get();
        $kws = [];
        if ($kw) {
            $kws = explode(',', $kw);
        }
        $qry = Model_has_permission::query();
        $qry->whereRaw('1 = 1');
        $vkw = '';
        if ($kws) {
        ...
        $info = Dbinfo::where('table_name', 'model_has_permissions')->first();
        //---addition----
        $method = [];
        $domains = [];
...
protected function model(){
    return new Model_has_permission();
}
