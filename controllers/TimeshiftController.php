...
class TimeshiftController extends BaseController {
...
public function all($page = 1, $perpage = 10, $kw = '', $ajax = 0){
        ...
        $columns = Column::where('table_id', 'timeshifts')->orderBy('sort', 'asc')->get();
        $kws = [];
        if ($kw) {
            $kws = explode(',', $kw);
        }
        ...
        $info = Dbinfo::where('table_name', 'timeshifts')->first();
        //---addition----
        $method = [];
        $domains = [];
